<?PHP
	...
	<center>
		<table style="width:500px;">
			...
			function list_menu($sub, $u_id, $n, $margin, $conn){
					$s_m = MYSQLI_QUERY($conn,"SELECT * FROM product_cat WHERE sub_id = '".$sub."' AND l_id='1' AND tip='0' ORDER BY ordering ASC");
					if(mysqli_num_rows($s_m) != 0){
						while($n_m = MYSQLI_FETCH_ASSOC($s_m)){
							?>
							<tr>
								<td style="background:#F3F3F3; padding-left:<?PHP echo $margin?>px;"><a href="?menu=product&catid=<?PHP echo $n_m['u_id'];?>"><?PHP echo $n_m['name'];?></a></td>
							</tr>
							<tr>
								<td height="2"></td>
							</tr>
	...
	$s_cat_title = MYSQLI_QUERY($connection,"SELECT * FROM product_cat WHERE l_id='1' AND tip='0' AND u_id=".$catid);
	$n_cat_title = MYSQLI_FETCH_ASSOC($s_cat_title);
	?>
	<table cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td><a href="?menu=product"><u><b><?PHP echo $n_cat_title['name'];?></b></u></a></td>
		</tr>
	</table>
	<table>
		<!-- HEAD -->
		...
		<tbody>
			<?PHP				
				...
				$s_say_menu = MYSQLI_QUERY($connection,"SELECT * FROM product WHERE cat_id='".$catid."' AND l_id='1' AND tip='0' ORDER BY ordering DESC");
				...
				$s_m = MYSQLI_QUERY($connection,"SELECT * FROM product WHERE cat_id='".$catid."' AND l_id='1' AND tip='0' ORDER BY ordering DESC limit ".$from.", ".$page_count."");
				while($n_m = MYSQLI_FETCH_ASSOC($s_m)){
					?>
					...
					<tr>
						...
						<td>
							...
							<a href="?menu=product_photo&p_id=<?PHP echo $n_m['u_id'];?>" title="şəkil əlavə et">
							 ...
							 <a href="?menu=product&catid=<?PHP echo $n_m['cat_id'];?>&mod=edit&id=<?PHP echo $n_m['u_id'];?>" title="Düzəliş et">
								<img src="images/pencil.png" alt="Düzəliş et" />
							 </a>&nbsp;
							 <a href="?menu=product&catid=<?PHP echo $n_m['cat_id'];?>&mod=delete&id=<?PHP echo $n_m['u_id'];?>" title="Sil">
			...
			<tr>
				<td colspan="9">
					<div class="pagination">
						...
						?>
						<a href="?menu=product&catid=<?PHP echo $catid;?>&page=1" title="Birinci səhifə">&laquo; Birinci</a>
						<a href="?menu=product&catid=<?PHP echo $catid;?>&page=<?PHP echo $page-1;?>" title="Geri"> &laquo; Geri </a>
						<?PHP
						}	
							$paging = '';
							$kichikdir = 5;
							...
							$oterefbuteref=3;
								if($bolme_sayi <= $boyukdur){
									...
									$paging .= "<a href='?menu=product&catid=".$catid."&page=".$nomre."' class='number  "; 
								...
								elseif($bolme_sayi > $kichikdir && $page <=$boyukdur && $page>=$kichikdir ){
									...
									$paging.= "....<a href='?menu=product&catid=".$catid."&page=".$bolme_sayi."' class='number  "; 
								...
								elseif($bolme_sayi > $boyukdur && $page >=$boyukdur && $page <=($bolme_sayi-$kichikdir)  ){
									$paging.= "<a href='?menu=product&catid=".$catid."&page=1' class='number  "; 
											if($bolme_sayi == $page) $paging.="current";
											$paging.="' title='1'>1</a>...";
						...
						?>
						<a href="?menu=product&catid=<?PHP echo $catid;?>&page=<?PHP echo $page+1;?>" title="Irəli"> İrəli &raquo;</a>
						<a href="?menu=product&catid=<?PHP echo $catid;?>&page=<?PHP echo $bolme_sayi;?>" title="Sonuncu səhifə">Sonuncu &raquo;</a>
						<?PHP
						}
						?>
					</div> <!-- pagination -->
