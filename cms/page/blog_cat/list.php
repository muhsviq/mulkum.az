<?PHP
	...
	if(isset($save)){
			...
			foreach($array as $k=>$vpppal) {
				foreach($vpppal as $ooo){
					$supdate = MYSQLI_QUERY($connection,"UPDATE blog_cat SET ordering='".$ooo."' WHERE u_id ='".$k."' ");
				}
			}
			if($supdate){
	...
	if(isset($delete)){
			...
			{
				$s_del = MYSQLI_QUERY($connection,"DELETE FROM blog_cat WHERE u_id = '".$chek[$n]."'");
			}
			if($s_del){
	...
	if(isset($aktiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE blog_cat SET status='0' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	if(isset($passiv)){
			...
			for($n=0; $n<count($chek); $n++)
			{
				$supdate = MYSQLI_QUERY($connection,"UPDATE blog_cat SET status='1' WHERE u_id = '".$chek[$n]."'");
			}
			if($supdate){
	...
	<!-- HEAD -->
	 ...
	 <!-- BODY -->
		...
		<?PHP 
				...
				$s_say_menu = MYSQLI_QUERY($connection,"SELECT * FROM blog_cat WHERE l_id='1' AND tip='0' ORDER BY ordering ASC");
				...
				function list_menu($sub, $u_id, $n, $from, $page_count,$margin,$conn, $lng, $dil){
					$s_m = MYSQLI_QUERY($conn,"SELECT * FROM blog_cat WHERE sub_id = '".$sub."' AND l_id='1' AND tip='0' ORDER BY ordering ASC limit ".$from.", ".$page_count."");
							...
							<tr>
								...
								<td>
									<!-- Icons -->
									 ...
									 <a href="?menu=blog_cat&mod=edit&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Düzəliş et'][$lng]; ?>">
										<img src="images/pencil.png" alt="Düzəliş et" />
									 </a>
									 <a href="?menu=blog_cat&mod=delete&id=<?PHP echo $n_m['u_id'];?>" title="<?php echo $dil['Sil'][$lng]; ?>">
			...
			<tr>
				<td colspan="6">
					<div class="pagination">
						...
						?>
						<a href="?menu=blog_cat&page=1" title="Birinci səhifə">&laquo; <?php echo $dil['Birinci'][$lng]; ?></a>
						<a href="?menu=blog_cat&page=<?PHP echo $page-1;?>" title="Geri"> &laquo; <?php echo $dil['Geri'][$lng]; ?> </a>
						<?PHP
						}	
							$paging = '';
							$kichikdir = 5;
							...
							$oterefbuteref=3;
								if($bolme_sayi <= $boyukdur){
									...
									$paging .= "<a href='?menu=blog_cat&page=".$nomre."' class='number  "; 
								...
								elseif($bolme_sayi > $kichikdir && $page <=$boyukdur && $page>=$kichikdir ){
									...
									$paging.= "....<a href='?menu=blog_cat&page=".$bolme_sayi."' class='number  "; 
								...
								elseif($bolme_sayi > $boyukdur && $page >=$boyukdur && $page <=($bolme_sayi-$kichikdir)  ){
									$paging.= "<a href='?menu=blog_cat&page=1' class='number  "; 
											if($bolme_sayi == $page) $paging.="current";
											$paging.="' title='1'>1</a>...";
						...
						?>
						<a href="?menu=blog_cat&page=<?PHP echo $page+1;?>" title="Irəli"> <?php echo $dil['İrəli'][$lng]; ?> &raquo;</a>
						<a href="?menu=blog_cat&page=<?PHP echo $bolme_sayi;?>" title="Sonuncu səhifə"><?php echo $dil['Sonuncu'][$lng]; ?> &raquo;</a>
						<?PHP
						}
						?>
					</div> <!-- pagination -->
