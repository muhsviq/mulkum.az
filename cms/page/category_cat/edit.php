...
$tip=0;
if(!empty($avatar)){
$delphoto = MYSQLI_QUERY($connection,"SELECT photo FROM gal_cat WHERE u_id='".$idx."' AND l_id='1'");
...
unlink('../'.$delphoto22['photo']);
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			$text= $_POST['text'.$i];
			if(empty($avatar)){
			$update = MYSQLI_QUERY($connection,"UPDATE gal_cat SET `sub_id`='$kateqoriya', `name` = '$name', `text` = '$text', `status` = '$active', `url_tag` = '$url_tag', `title` = '$title', `keyword` = '$keyword', `description` = '$description' WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
			else{
			$update = MYSQLI_QUERY($connection,"UPDATE gal_cat SET `sub_id`='$kateqoriya', `name` = '$name', `text` = '$text', `status` = '$active', `url_tag` = '$url_tag', `title` = '$title', `keyword` = '$keyword', `description` = '$description',`photo` = '$avatar' WHERE u_id='".$idx."' AND l_id='".$i."'");
			}
		}
		if($update){
		...
		<form action="" method="POST" enctype="multipart/form-data">
			<table cellpadding="0" cellspacing="0" border="0" align="center">
				...
				<tr>
					<td>
					<?PHP
					$s_redakte1=mysqli_query($connection,'SELECT * FROM gal_cat WHERE u_id='.$idx.' AND l_id=1');
					...
					?>
						...
						<?PHP
							$s_sub = mysqli_query($connection,'SELECT * FROM gal_cat WHERE l_id=1 ORDER BY ordering ASC ');
					 ...
					 <link rel="stylesheet" href="js/jquery-ui.css">
						...
						<div id="tabs">
								...
								<?PHP
									...
									while($nl = MYSQLI_FETCH_ARRAY($sl)){
									$dm=mysqli_query($connection,'SELECT * FROM gal_cat WHERE u_id='.$idx.' AND l_id='.$nl['id'].' limit 1');
