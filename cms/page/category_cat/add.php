<?PHP
	if(@$send){
		...
		$ddd=mysqli_query($connection,'select max(u_id) as uid from gal_cat');
		$bbb=MYSQLI_FETCH_ARRAY($ddd);
		$u_id=$bbb['uid'];
		$u_id++;
		
		$zzz=mysqli_query($connection,'select max(ordering) as uid from gal_cat');
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			@$insert = MYSQLI_QUERY($connection,"INSERT INTO `gal_cat` (
				`sub_id`,
				`name`, 
				`text`, 
				`l_id`, 
