<?PHP
	if(@$send){
		...
		$ddd=mysqli_query($connection,'select max(u_id) as uid from product_cat');
		$bbb=MYSQLI_FETCH_ARRAY($ddd);
		$u_id=$bbb['uid'];
		$u_id++;
		
		$zzz=mysqli_query($connection,'select max(ordering) as uid from product_cat');
		...
		for($i=1; $i<= $hidsay; $i++ ){
			...
			@$insert = MYSQLI_QUERY($connection,"INSERT INTO `product_cat` (
				`sub_id`,
				`name`, 
				`text`, 
				`l_id`, 
						...
						<div id="tabs">
								...
								<table>
									...
									<tr>
										<td>Cat.: </td>
										<td>
											<select style="width:500px;border-radius:5px;" name="url_tag<?PHP echo $nl['id']; ?>">
												<option value=""></option>
									...
									<tr>
										...
										<td><input name="url_tag2<?PHP echo $nl['id']; ?>" style="width:500px;border-radius:5px;"></td>
									</tr>
									<tr>
