	...
	if(@$send){
		...
		if(!empty($_FILES['img']['tmp_name'])){
			...
			function getMax_Order($connection) {
				$q="select max(ordering) from xeberler_photo";
				$res = mysqli_query($connection,$q);
				$b=mysqli_fetch_array($res);
				return $b[0];
			}
			$getmax_order = getMax_Order($connection)+1;
				@$text= $_POST['text'];
				$insert = MYSQLI_QUERY($connection,"INSERT INTO xeberler_photo (photo, s_id, ordering, u_id,  text, p_id ) values('".$foto_insert."', '0', '".$getmax_order."', '".$getmax."','".$text."' ,'".$p_id."' )");
